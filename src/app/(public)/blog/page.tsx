      ...
      <div className="mb-16">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 border-b border-border pb-8">
          ...
          <div className="relative w-full md:w-80">
            <Search strokeWidth={1.5} className="absolute left-4 top-1/2 -translate-y-1/2 h-4 w-4 text-muted-foreground/80" />
            <Input
              placeholder="Search archive..."
              value={searchQuery}
              onChange={(e) => setSearchQuery(e.target.value)}
              className="pl-11 h-12 bg-gray-50 border-border text-foreground rounded-lg focus-visible:ring-1 focus-visible:ring-gray-300 text-xs font-semibold placeholder:text-muted-foreground/80"
            />
          </div>
        </div>
      </div>
      ...
      {loading ? (
        ...
        <div className="grid gap-8 md:grid-cols-2 lg:grid-cols-3">
          ...
          {filteredPosts.length === 0 && (
            <div className="col-span-full text-center py-24 text-muted-foreground bg-gray-50 rounded-xl border border-border border-dashed">
...
function ArticleCard({ post, index }: { post: BlogPost, index: number }) {
  ...
  return (
    <Link href={`/blog/${post.slug}`} className="group relative flex flex-col rounded-xl border border-border bg-background overflow-hidden hover:shadow-lg hover:-translate-y-1 transition-all duration-300">
      <div className="aspect-[16/10] w-full bg-gray-100 overflow-hidden relative border-b border-gray-100">
        ...
        <div className="absolute top-4 left-4 flex gap-2">
          {post.tags.slice(0, 1).map(tag => (
            <Badge key={tag} variant="secondary" className="bg-background text-foreground border-border text-[10px] h-6 px-3 uppercase tracking-wider font-bold shadow-sm">
              {tag}
            </Badge>
          ))}
        </div>
      ...
      <div className="p-6 flex flex-col flex-1">
        <div className="text-[11px] font-bold text-muted-foreground/80 uppercase tracking-widest mb-3">{date}</div>
        <h3 className="text-xl font-bold text-foreground mb-3 line-clamp-2 leading-snug tracking-tight group-hover:text-gray-600 transition-colors">
          {post.title}
        </h3>
        <p className="text-sm text-muted-foreground font-medium line-clamp-2 mb-6 leading-relaxed">
          {post.excerpt}
        </p>

        <div className="flex items-center text-xs font-bold text-foreground mt-auto">
