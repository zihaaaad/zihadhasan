...
export default function BookDetailsClient({ book, initialPurchaseStatus }: BookDetailsClientProps) {
 ...
 animate={{ opacity: 1, scale: 1 }}
              className="relative aspect-[3/4] w-full rounded-[2.5rem] overflow-hidden border border-border bg-gray-50"
 >
 {book.imageUrl ? (
 ...
 <div className="grid sm:grid-cols-2 gap-6">
          <div className="bg-background border border-border p-8 flex flex-col gap-4 rounded-3xl shadow-sm">
            <span className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest">E-Book Edition</span>
            <div className="flex items-end gap-2">
              <span className="text-3xl font-bold text-foreground tracking-tight">{formatCurrency(book.price)}</span>
              <span className="text-muted-foreground text-xs mb-1 line-through">{formatCurrency(book.price + 100)}</span>
            </div>
            <ul className="text-xs font-semibold text-muted-foreground space-y-2 mt-2">
 ...
 {book.hardcopyPrice && (
            <div className="bg-background border border-border p-8 flex flex-col gap-4 rounded-3xl shadow-sm">
              <span className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest">Hardcopy Print</span>
              <div className="flex items-end gap-2">
                <span className="text-3xl font-bold text-foreground tracking-tight">{formatCurrency(book.hardcopyPrice)}</span>
              </div>
              <ul className="text-xs font-semibold text-muted-foreground space-y-2 mt-2">
 ...
 <Button size="lg" className="rounded-xl h-14 px-10 text-[11px] font-bold uppercase tracking-[0.2em] bg-background text-foreground hover:bg-neutral-200">
 Read Online Now <BookOpen className="ml-2 h-4 w-4" />
 </Button>
 </Link>
 ) : (
 <>
              <Button size="lg" onClick={handlePurchaseClick} className="rounded-xl h-14 px-10 text-[11px] font-bold uppercase tracking-[0.2em] bg-primary text-primary-foreground hover:bg-primary/90">
