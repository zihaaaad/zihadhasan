...
export default function PublicEventsPage() {
 ...
 <p className="text-gray-500 font-medium max-w-2xl mx-auto uppercase tracking-widest text-[10px]">
...
function EventCard({ event, onRegister, index }: { event: Event, onRegister: () => void, index: number }) {
 ...
 className="group relative flex flex-col rounded-[2rem] border border-white/[0.05] bg-white/[0.02] overflow-hidden hover:border-gray-200 transition-all duration-700 h-full"
 >
 ...
 <Badge variant="secondary" className="bg-white text-white/70 border-gray-200 text-[9px] font-bold uppercase tracking-widest px-3 h-6">
 ...
 <p className="text-sm text-gray-500 font-medium line-clamp-2 mb-8 leading-relaxed">{event.description}</p>
 ...
 onClick={onRegister}
 disabled={isSoldOut}
 className={`w-full h-12 rounded-xl text-[11px] font-bold uppercase tracking-[0.2em] transition-all duration-500 ${isSoldOut ? 'bg-white text-gray-800 border-gray-200' : 'bg-white text-black hover:bg-neutral-200'}`}
 >
...
function RegistrationModal({ event, open, onOpenChange, onSuccess }: { event: Event | null, open: boolean, onOpenChange: (open: boolean) => void, onSuccess: () => void }) {
 ...
 {!isFree && (
 <div className="bg-white border border-gray-200 p-4 rounded-lg space-y-3 mb-2">
 <div className="space-y-2">
 <div className="flex justify-between items-center text-sm">
 <span className="text-pink-400 font-bold">bKash (Send Money):</span>
 <span className="font-mono bg-gray-50 px-2 py-1 rounded select-all">{paymentInfo.bkash || "Not Set"}</span>
 </div>
 <div className="flex justify-between items-center text-sm">
 <span className="text-orange-400 font-bold">Nagad (Send Money):</span>
 <span className="font-mono bg-gray-50 px-2 py-1 rounded select-all">{paymentInfo.nagad || "Not Set"}</span>
 </div>
 </div>
 ...
 <div key={idx} className="text-xs bg-gray-50 p-2 rounded border border-gray-200 space-y-1">
 ...
 <Input id="name" {...register("name")} className="bg-white border-gray-200 text-white" />
 {errors.name && <p className="text-xs text-red-500">{errors.name.message}</p>}
 </div>
 <div className="grid gap-2">
 <Label htmlFor="phone" className="text-white">Phone Number</Label>
 <Input id="phone" {...register("phone")} className="bg-white border-gray-200 text-white" placeholder="017..." />
 {errors.phone && <p className="text-xs text-red-500">{errors.phone.message}</p>}
 </div>
 <div className="grid gap-2">
 <Label htmlFor="email" className="text-white">Email Address</Label>
 <Input id="email" {...register("email")} className="bg-white border-gray-200 text-white" />
 {errors.email && <p className="text-xs text-red-500">{errors.email.message}</p>}
 </div>
 {!isFree && (
 <div className="grid gap-2">
 <Label htmlFor="trxId" className="text-white">Transaction ID (TrxID)</Label>
 <Input id="trxId" {...register("trxId")} className="bg-white border-gray-200 text-white uppercase font-mono tracking-widest" placeholder="9G7..." />
 ...
 <div className="bg-white border border-gray-200 rounded-lg p-4 w-full cursor-pointer hover:bg-white transition-colors"
 ...
 <Button
 variant="outline"
 className="w-full border-gray-200 hover:bg-white hover:text-white"
 onClick={() => {
