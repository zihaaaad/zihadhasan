...
export function ProjectsClient({ initialProjects }: { initialProjects: Project[] }) {
  // Seeded from the build-time fetch in page.tsx so the case studies are in the
  // prerendered HTML - crawlers used to see an empty section here, because this
  // list only ever existed after a client-side Firestore round trip.
  const [projects, setProjects] = useState<Project[]>(initialProjects);
  const [projectsLoading, setProjectsLoading] = useState(initialProjects.length === 0);

  // GitHub Repos state
  // Baked in at build time by scripts/fetch-github-repos.js - never refetched
  // in the browser, so this needs no setter.
  const repos = githubReposData as unknown as GitHubRepo[];
  ...
  // Refresh in the background so projects added since the last build still show
  // up for visitors, without the prerendered list flashing empty first.
  useEffect(() => {
    let cancelled = false;

    CMSService.getProjects()
      .then((data) => {
        if (!cancelled) setProjects(applyProjectCopyOverrides(data));
      })
  ...
  return (
    <div className="min-h-screen bg-background text-foreground pt-32 pb-24 font-sans">
      <div className="container mx-auto px-4 lg:px-8 max-w-7xl">
        {/* Header Section */}
        <div className="mb-12 border-b border-border pb-10 flex flex-col md:flex-row gap-8 items-center justify-between">
          <div>
