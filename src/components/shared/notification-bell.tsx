    ...
    <Popover open={isOpen} onOpenChange={setIsOpen}>
      <PopoverTrigger asChild>
        <Button
          ...
          className="relative text-gray-500 hover:text-black hover:bg-gray-100 rounded-full h-9 w-9 transition-colors"
        >
          <Bell strokeWidth={1.5} className="h-5 w-5" />
          {unreadCount > 0 && (
            <span className="absolute top-0 right-0 h-2.5 w-2.5 rounded-full bg-black border border-white animate-pulse" />
          )}
        </Button>
      </PopoverTrigger>
      <PopoverContent className="w-80 p-0 bg-white border border-gray-100 shadow-xl rounded-2xl text-black overflow-hidden" align="end">
            ...
            <div className="flex items-center gap-1">
              {unreadCount > 0 && (
                <Button
                  variant="ghost"
                  size="sm"
                  className="h-6 text-[10px] px-2 text-gray-500 hover:text-black hover:bg-gray-100 rounded-md"
                ...
                >
                  <div className="flex gap-3">
                    <div className="flex-1 space-y-1">
                      <p className={cn("text-sm", !notification.read ? "text-black font-bold" : "text-gray-600 font-medium")}>
                        {notification.title}
                      </p>
                    ...
                    {!notification.read && (
                      <div className="flex flex-col justify-center">
                        <Button
                          variant="ghost"
                          size="icon"
                          className="h-6 w-6 text-gray-400 hover:text-black hover:bg-gray-200 rounded-full"
                          onClick={(e) => {
                            e.stopPropagation();
                            markAsRead(notification.id);
                          }}
        ...
        <div className="p-2 border-t border-gray-50 bg-gray-50/50">
          <Link href="/notifications" onClick={() => setIsOpen(false)}>
            <Button variant="ghost" className="w-full h-8 text-xs font-semibold text-gray-500 hover:text-black hover:bg-gray-100 rounded-md">
