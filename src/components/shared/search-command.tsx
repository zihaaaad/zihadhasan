...
export function SearchCommand() {
 ...
 <DialogContent className="p-0 gap-0 bg-black border-gray-200 sm:max-w-[550px] shadow-2xl rounded-3xl overflow-hidden">
 <DialogHeader className="p-6 border-b border-gray-200 bg-white/[0.02]">
 <DialogTitle className="sr-only">Search</DialogTitle>
 <div className="flex items-center gap-4 px-2">
 <Search strokeWidth={1.5} className="h-5 w-5 text-gray-500" />
 <Input
 value={query}
 onChange={(e) => setQuery(e.target.value)}
 placeholder="Search everything..."
 className="border-0 bg-transparent text-xl focus-visible:ring-0 placeholder:text-gray-800 h-auto p-0 text-white font-medium tracking-tight"
 autoFocus
 />
 ...
 className="w-full text-left px-5 py-4 rounded-2xl hover:bg-white active:bg-white transition-all duration-300 group flex items-start gap-5"
 >
 <div className={cn(
 "mt-1 p-2.5 rounded-xl bg-white text-gray-500 group-hover:text-white group-hover:bg-white transition-all duration-500"
 )}>
 {getIcon(result.type)}
 </div>
 ...
 <span className="text-[9px] uppercase font-black tracking-[0.2em] text-gray-800 group-hover:text-gray-600 self-center bg-white px-2 py-1 rounded-md transition-colors">
 {result.type}
 </span>
 </button>
 ))}
 ...
 <p className="text-sm text-gray-800 font-medium tracking-wide uppercase">
 ...
 <div className="p-4 border-t border-gray-200 bg-white/[0.03] flex justify-between items-center px-6">
 <div className="flex items-center gap-2">
 <span className="text-[10px] font-bold text-gray-800 uppercase tracking-widest">Navigation</span>
 <div className="h-px w-8 bg-white" />
 </div>
 <div className="flex gap-3">
 <kbd className="pointer-events-none inline-flex h-6 select-none items-center gap-1 rounded-md border border-gray-200 bg-black px-2 font-mono text-[10px] font-bold text-gray-500 uppercase">
 esc
 </kbd>
