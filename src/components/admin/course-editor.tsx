...
export function CourseEditor({ course, onSave, onCancel }: CourseEditorProps) {
 ...
 <Button variant="ghost" onClick={onCancel} className="text-gray-500 hover:text-white h-12 rounded-xl text-[10px] font-bold uppercase tracking-widest">Cancel</Button>
 ...
 <Label className="text-[10px] font-bold uppercase tracking-widest text-gray-500 ml-1">Academic Title</Label>
 <Input
 value={title}
 onChange={e => setTitle(e.target.value)}
 className="bg-gray-50 border-gray-200 text-white h-12 rounded-xl font-bold uppercase tracking-widest text-[11px] px-5"
 placeholder="MASTERING GENERATIVE DESIGN"
 />
 </div>
 <div className="space-y-2">
 <Label className="text-[10px] font-bold uppercase tracking-widest text-gray-500 ml-1">Educational Narrative</Label>
 <Textarea
 value={description}
 onChange={e => setDescription(e.target.value)}
 className="bg-gray-50 border-gray-200 text-white min-h-[120px] rounded-2xl p-5 leading-relaxed"
 ...
 <Button size="sm" variant="outline" onClick={handleAddLesson} className="border-gray-200 hover:bg-white rounded-lg h-9 text-[10px] font-bold uppercase tracking-widest px-4">
 <Plus strokeWidth={1.5} className="h-3.5 w-3.5 mr-2" /> Add Module
 </Button>
 </CardHeader>
 <CardContent className="space-y-4">
 {lessons.length === 0 && (
 <div className="text-center py-16 text-gray-800 border border-dashed border-gray-200 rounded-2xl uppercase tracking-[0.2em] font-black text-[10px]">
 System awaiting curriculum input.
 </div>
 )}
 {lessons.map((lesson, index) => (
 <div key={index} className="flex gap-4 bg-white/[0.01] p-6 rounded-2xl border border-gray-200 group transition-all hover:bg-white/[0.03]">
 ...
 className="h-10 bg-gray-50 border-gray-200 rounded-xl font-bold text-xs px-4"
 />
 </div>
 <div className="w-24">
 ...
 className="pl-10 h-10 bg-gray-50 border-gray-200 rounded-xl text-[10px] font-mono"
 ...
 <div className="flex items-center justify-between bg-white/[0.03] p-4 rounded-xl border border-gray-200">
 <div className="space-y-0.5">
 <Label className="text-xs font-bold uppercase tracking-widest">Public Status</Label>
 <p className="text-[9px] text-gray-500 font-bold uppercase tracking-widest">LMS Visibility</p>
 </div>
 <Switch checked={published} onCheckedChange={setPublished} />
 </div>

 <div className="space-y-4">
 <Label className="text-[10px] font-bold uppercase tracking-widest text-gray-500 ml-1">Monetization Protocol</Label>
 <RadioGroup
 value={pricingType}
 onValueChange={(v: 'free' | 'paid') => setPricingType(v)}
 className="flex flex-col gap-3"
 >
 <div className="flex items-center space-x-3 bg-white/[0.01] p-3 rounded-xl border border-gray-200">
 <RadioGroupItem value="free" id="r-free" className="border-gray-200 text-white" />
 <Label htmlFor="r-free" className="text-xs font-bold uppercase tracking-widest cursor-pointer flex-1">Open Access</Label>
 </div>
 <div className="flex items-center space-x-3 bg-white/[0.01] p-3 rounded-xl border border-gray-200">
 <RadioGroupItem value="paid" id="r-paid" className="border-gray-200 text-white" />
 ...
 <Label className="text-[10px] font-bold uppercase tracking-widest text-gray-500 ml-1">Value (BDT)</Label>
 <Input
 type="number"
 value={price}
 onChange={e => setPrice(Number(e.target.value))}
 className="bg-gray-50 border-gray-200 text-white h-12 rounded-xl font-bold px-5"
 ...
 <AlertDialogContent className="bg-white border-gray-200 text-white rounded-3xl p-8">
 <AlertDialogHeader>
 ...
 <AlertDialogCancel onClick={handleDiscardDraft} className="border-gray-200 hover:bg-white text-gray-500 hover:text-white rounded-xl h-12 px-6 text-[10px] font-bold uppercase tracking-widest">
 Discard
 </AlertDialogCancel>
