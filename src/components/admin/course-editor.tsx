...
export function CourseEditor({ course, onSave, onCancel }: CourseEditorProps) {
 ...
 <div className="flex gap-3">
        <Button variant="ghost" onClick={onCancel} className="text-muted-foreground hover:text-foreground h-12 rounded-xl text-[10px] font-bold uppercase tracking-widest">Cancel</Button>
        <Button onClick={handleSubmit} disabled={loading} className="bg-primary text-primary-foreground hover:bg-primary/90 h-12 px-8 rounded-xl text-[10px] font-bold uppercase tracking-widest transition-all">
 ...
 <div className="md:col-span-2 space-y-10">
        <Card className="bg-background border-border text-foreground rounded-[2rem] p-4 shadow-sm">
 <CardHeader>
 ...
 onChange={e => setTitle(e.target.value)}
                className="bg-gray-50 border-border text-foreground h-12 rounded-xl font-bold uppercase tracking-widest text-[11px] px-5"
 placeholder="MASTERING GENERATIVE DESIGN"
 />
 </div>
 <div className="space-y-2">
 <Label className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground ml-1">Educational Narrative</Label>
 <Textarea
 value={description}
 onChange={e => setDescription(e.target.value)}
                className="bg-gray-50 border-border text-foreground min-h-[120px] rounded-2xl p-5 leading-relaxed"
 ...
 <Button size="sm" variant="outline" onClick={handleAddLesson} className="border-border hover:bg-background rounded-lg h-9 text-[10px] font-bold uppercase tracking-widest px-4">
 <Plus strokeWidth={1.5} className="h-3.5 w-3.5 mr-2" /> Add Module
 </Button>
 </CardHeader>
 <CardContent className="space-y-4">
 {lessons.length === 0 && (
 <div className="text-center py-16 text-gray-800 border border-dashed border-border rounded-2xl uppercase tracking-[0.2em] font-black text-[10px]">
 System awaiting curriculum input.
 </div>
 )}
 {lessons.map((lesson, index) => (
              <div key={index} className="flex gap-4 bg-gray-50 p-6 rounded-2xl border border-border group transition-all hover:bg-background">
 <div className="mt-3 text-neutral-700 cursor-grab group-hover:text-gray-600 transition-colors">
 <GripVertical strokeWidth={1.5} className="h-5 w-5" />
 </div>
 <div className="flex-1 space-y-4">
 <div className="flex gap-4">
 <div className="flex-1">
 <Label className="text-[9px] font-bold uppercase tracking-widest text-gray-800 ml-1">Module Title</Label>
 <Input
 value={lesson.title}
 onChange={e => handleLessonChange(index, "title", e.target.value)}
 className="h-10 bg-gray-50 border-border rounded-xl font-bold text-xs px-4"
 />
 </div>
 <div className="w-24">
 <Label className="text-[9px] font-bold uppercase tracking-widest text-gray-800 ml-1">Sequence</Label>
 <Input
 type="number"
 value={lesson.order}
 ...
 onChange={e => handleLessonChange(index, "videoUrl", e.target.value)}
                          className="pl-10 h-10 bg-background border-border rounded-xl text-[10px] font-mono text-foreground"
 ...
 <p className="text-[9px] text-muted-foreground font-bold uppercase tracking-widest">LMS Visibility</p>
 </div>
 <Switch checked={published} onCheckedChange={setPublished} />
 </div>

 <div className="space-y-4">
 <Label className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground ml-1">Monetization Protocol</Label>
 <RadioGroup
 value={pricingType}
 onValueChange={(v: 'free' | 'paid') => setPricingType(v)}
 className="flex flex-col gap-3"
 >
                <div className="flex items-center space-x-3 bg-background p-3 rounded-xl border border-border">
                  <RadioGroupItem value="free" id="r-free" className="border-border text-foreground" />
                  <Label htmlFor="r-free" className="text-xs font-bold uppercase tracking-widest cursor-pointer flex-1">Open Access</Label>
                </div>
                <div className="flex items-center space-x-3 bg-background p-3 rounded-xl border border-border">
                  <RadioGroupItem value="paid" id="r-paid" className="border-border text-foreground" />
 ...
 <Label className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground ml-1">Value (BDT)</Label>
 <Input
 type="number"
 value={price}
 onChange={e => setPrice(Number(e.target.value))}
                  className="bg-gray-50 border-border text-foreground h-12 rounded-xl font-bold px-5"
 ...
 <AlertDialog open={showDraftDialog} onOpenChange={setShowDraftDialog}>
        <AlertDialogContent className="bg-background border-border text-foreground rounded-3xl p-8">
 <AlertDialogHeader>
 ...
 <AlertDialogFooter className="mt-8 gap-3">
            <AlertDialogCancel onClick={handleDiscardDraft} className="border-border hover:bg-gray-50 text-muted-foreground hover:text-foreground rounded-xl h-12 px-6 text-[10px] font-bold uppercase tracking-widest">
              Discard
            </AlertDialogCancel>
            <AlertDialogAction onClick={handleRestoreDraft} className="bg-primary text-primary-foreground hover:bg-primary/90 rounded-xl h-12 px-8 text-[10px] font-bold uppercase tracking-widest">
 Restore Data
 </AlertDialogAction>
 </AlertDialogFooter>
 </AlertDialogContent>
