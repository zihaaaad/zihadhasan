...
export function BlogEditor({ initialData, localStorageKey = 'blog_draft_new' }: BlogEditorProps) {
 ...
 {/* Header / Actions */}
 <div className="flex items-center justify-between sticky top-0 bg-gray-50 z-10 py-4 border-b border-border">
 <Button variant="ghost" onClick={() => router.back()} className="text-muted-foreground/80">
 ...
 <Button onClick={handleSave} disabled={submitting} className="bg-primary text-foreground hover:bg-primary/90">
 ...
 {/* Meta Inputs */}
 <div className="grid gap-4 p-6 bg-background border border-border rounded-xl">
 <div className="grid gap-2">
 <Label className="text-primary-foreground">Post Title</Label>
 <Input
 value={title}
 onChange={(e) => setTitle(e.target.value)}
 placeholder="Enter a catchy title"
 className="text-lg font-bold bg-transparent border-border text-primary-foreground"
 />
 </div>

 <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
 <div className="grid gap-2">
 <Label className="text-primary-foreground">Slug</Label>
 <Input
 value={slug}
 onChange={(e) => setSlug(e.target.value)}
 className="bg-background border-border text-gray-300 font-mono text-sm"
 />
 </div>
 <div className="grid gap-2">
 <Label className="text-primary-foreground">Tags (comma separated)</Label>
 <Input
 value={tags}
 onChange={(e) => setTags(e.target.value)}
 placeholder="Next.js, React, Design"
 className="bg-background border-border text-primary-foreground"
 />
 </div>
 </div>

 <div className="grid gap-2">
 <Label className="text-primary-foreground">Excerpt (SEO Description)</Label>
 <Input
 value={excerpt}
 ...
 {/* Editor Toolbar */}
 <div className="sticky top-[73px] z-10 flex flex-wrap gap-1 p-2 bg-background border border-border rounded-t-xl ">
 ...
 {/* Editor Content */}
 <div className="min-h-[500px] p-6 bg-primary border border-t-0 border-border rounded-b-xl text-primary-foreground">
 <EditorContent editor={editor} />
 </div>
 </div>

 <AlertDialog open={showDraftDialog} onOpenChange={setShowDraftDialog}>
 <AlertDialogContent className="bg-zinc-900 border-border text-primary-foreground">
 <AlertDialogHeader>
 ...
 <AlertDialogFooter>
 <AlertDialogCancel onClick={handleDiscardDraft} className="border-border hover:bg-background hover:text-primary-foreground text-muted-foreground/80">
 Discard
 </AlertDialogCancel>
 <AlertDialogAction onClick={handleRestoreDraft} className="bg-primary text-foreground hover:bg-primary/90">
 Restore Draft
 </AlertDialogAction>
 </AlertDialogFooter>
 </AlertDialogContent>
...
function ToolbarBtn({ onClick, isActive, icon }: { onClick: () => void, isActive: boolean, icon: React.ReactNode }) {
 ...
 <button
 onClick={onClick}
 className={`p-2 rounded hover:bg-background transition-colors ${isActive ? 'bg-primary/20 text-primary' : 'text-muted-foreground/80'}`}
 >
 {icon}
 </button>
 );
